...
import com.github.kamicloud.stub.core.segments.openapi.v3.*;
        ...
        templateStub.getModels().forEach(model -> {
            ...
            model.getParameters().forEach(parameterStub -> {
                ...
                if (parameterStub.getTypeSpec().equals(TypeSpec.MODEL)) {
                    propertySegment.setRef("#/components/schemas/" + parameterStub.getName().toUpperCamelCaseCase());
                }

                propertySegment.setNullable(parameterStub.hasAnnotation(Optional.class));

                if (parameterStub.hasAnnotation(Optional.class)) {
                    schemeSegment.getRequired().add(parameterStub.getName().toLowerCamelCase());
                }
                schemeSegment.getProperties().put(parameterStub.getName().toLowerCamelCase(), propertySegment);
            });

            componentSegment.getSchemas().put(model.getName().toUpperCamelCaseCase(), schemeSegment);
        });
