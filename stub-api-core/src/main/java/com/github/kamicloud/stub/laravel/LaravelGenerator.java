...
import com.github.kamicloud.stub.core.segments.php.*;
import com.github.kamicloud.stub.core.stubs.*;
import com.github.kamicloud.stub.core.utils.FileUtil;
import com.google.common.base.CaseFormat;
import com.github.kamicloud.stub.core.interfaces.PHPNamespacePathTransformerInterface;
...
//        config.getGenerators().getLaravel().getV
        ...
        String laravelPath = config.getGenerators().getLaravel().getPath();
    ...
    private void writeParameterGetter(ParameterStub parameterStub, ClassCombiner classCombiner, String prefix) {
        ...
        if (type.getSpec() == TypeSpec.MODEL) {
            returnType = parameterStub.getTypeSimpleName() + config.getGenerators().getLaravel().getDtoSuffix();
    ...
    private void writeGetAttributeMapMethod(String version, String methodName, LinkedList<ParameterStub> parameters, ClassCombiner classCombiner) {
        ...
        parameters.forEach((parameterStub) -> {
            ...
            if (isModel) {
                classCombiner.addUse("App\\Generated\\" + version + "\\" + config.getGenerators().getLaravel().getDtoFolder() +
                    "\\" + typeName + config.getGenerators().getLaravel().getDtoSuffix());
                rule = typeName + config.getGenerators().getLaravel().getDtoSuffix() + "::class";
            } else if (isEnum) {
